...
root_dir = '/home/liandz/eccv2018/IB_NN/baseline3/'
...
def test(dataset=test_loader, stage='test'):
    ...
    for c in range(1,10):
        layer_T_array_sample = torch.cat((layer_T_array_sample, layer_T_array[1000*c:1000*c+300]),0)
        label_test_matrix_sample = np.concatenate((label_test_matrix_sample, label_test_matrix[1000*c:1000*c+300]),0)
...
def test_train(dataset=test_train_loader, stage='test'):
    ...
    layer_T_array_sample = layer_T_array[0:300]
    label_train_matrix_sample = label_train_matrix[0:300]
    for c in range(1,10):
        layer_T_array_sample = torch.cat((layer_T_array_sample, layer_T_array[5000*c:5000*c+300]),0)
        label_train_matrix_sample = np.concatenate((label_train_matrix_sample, label_train_matrix[5000*c:5000*c+300]),0)
    
    #print(label_test_matrix_sample.shape)
    #print(layer_T_array_sample.data.cpu().numpy().shape)
    value_xt_train, value_ty_train = MI_cal_v2(label_train_matrix, layer_T_array.data.cpu().numpy(), args.mask)
    mi_xt_train.append(value_xt_train)
    mi_ty_train.append(value_ty_train)
...
xt_value_train_path = path + '/mi_xt_train.npy'
ty_value_train_path = path + '/mi_ty_train.npy'
...
np.save(xt_value_train_path, mi_xt_train)
np.save(ty_value_train_path, mi_ty_train)
